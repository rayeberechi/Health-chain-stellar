...
describe('InventoryForecasting Integration (SQLite)', () => {
    ...
    beforeAll(async () => {
        const module: TestingModule = await Test.createTestingModule({
            imports: [
                ...
                TypeOrmModule.forFeature([TestOrderEntity, InventoryEntity, UserEntity]),
            ],
            providers: [
        ...
        orderRepo = module.get<Repository<TestOrderEntity>>(getRepositoryToken(TestOrderEntity));
    ...
    it('should forecast low inventory using real database queries', async () => {
        ...
        expect(mockEventEmitter.emit).toHaveBeenCalledWith('inventory.low', expect.anything());
        expect(mockQueue.add).toHaveBeenCalledWith('recommend-donor-outreach', expect.objectContaining({
            bloodType: 'O-',
            region: region,
            urgency: 'high',
        }));
